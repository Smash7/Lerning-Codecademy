...
maze_file_name = r'Python3 Intermediate/The Great Robot Race (Project 6)/maze_data_1.csv'
...
# Add your code below!
robot_moves_conter = Counter(move[0] for move in robot_moves)

# Count the number of collisions by robot name
# Add your code below!
robot_collisions_conter = Counter(move[0]
                                  for move in robot_moves if move[2] == True)
...
bot_scores = []
# Add your code below! №9
for bot in bots:
  bot_scores.append(BotScoreData(
      bot.name, robot_moves_conter[bot.name], robot_collisions_conter[bot.name], robot_moves_conter[bot.name] + robot_collisions_conter[bot.name]))
